...
- Dotfiles repo root: !`git rev-parse --show-toplevel`
...
- Context の Dotfiles repo root に `dot_claude/settings.json` を結合したパスを Read ツールで読み込む（chezmoi ソースファイル）
...
**グローバル化の候補:**
- `permissions.allow` / `permissions.deny` / `permissions.ask` の全エントリ型:
  - `Bash(...)` — シェルコマンド
  - `WebFetch(domain:...)` — Web アクセス
  - `mcp__<server>__<tool>` — MCP ツール
  - `Skill(...)` — スキル
  - `Read(...)` — リポジトリ外パスの読み取り
...
- それ以外の場合: AskUserQuestion ツールで承認する番号（例: `1 3 5` または `all`）をユーザーに確認する
- 承認されたエントリを dotfiles リポジトリの `dot_claude/settings.json`（chezmoi ソース）の該当配列に Edit ツールで追記する
- 追記前に重複チェックを必ず行い、すでに存在するエントリは追加しない
- 適用後「N 件を追加しました。`git diff dot_claude/settings.json` で確認できます」と伝える
