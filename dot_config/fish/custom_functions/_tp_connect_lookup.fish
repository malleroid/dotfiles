function _tp_connect_lookup -d "Resolve local_port for a connect_db target via gi-no/infra env files"
    ...
    # An unknown target leaves con_settings unset and still exits 0, so an
    # empty value is the only signal that the lookup failed.
    set -l port (zsh -c "target_name='$target' source '$dir/$env_file' >/dev/null 2>&1; echo \$con_settings[4]" | string trim)
