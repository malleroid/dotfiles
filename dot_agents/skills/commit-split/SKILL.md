...
AskUserQuestion ツールで確認する（選択肢:「コミットする」「修正する」）。**「コミットする」の option には `preview` フィールドでコミット計画全文（各コミットのメッセージと対象ファイル一覧）を必ず添えること**（ダイアログ表示中は直前のテキストが見えないため、preview 内で確認できるようにする）。修正を求められた場合は計画を修正して再度 AskUserQuestion で確認する。
