...
Arguments には調査クエリ（自然言語の質問）が渡される想定。例:
- 「authProvider の初期実装はいつ追加された?」
- 「ユーザー認証バグを導入したコミットを特定」
- 「config/database.yml の変更履歴を追跡」

Arguments が空または曖昧な場合は、調査対象が判別できない旨をエラーとして報告し終了する（このスキルは parent の会話履歴にアクセスできないため、引数で明示されない情報は復元できない）。
...
1. **調査クエリを解析** — Arguments の内容を把握し、適切なコマンドを選択
