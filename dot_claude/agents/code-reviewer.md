...
11. レビュー報告時点では worktree を残す（追加質問でのコード参照用）。完了の通知を受けたら、モード 3 で自分が作成した worktree のみ削除条件を確認して `gwq remove` で削除する（モード 1・2 では何もしない）
...
レビュー完了後に追加質問が届いたら、レビュー時の文脈と残してある worktree を使って回答する。`finish`（または完了の通知）を受けたら後片付け（手順 11）を実行し、結果を 1 行で報告する（この後 agent は親に終了される）
...
- **category**: 指摘種別の短い slug（correctness / security / style 等）
