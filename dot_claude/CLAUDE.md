...
- 🔍 **調査済み**: このターンで実行した tool（Read / Grep / Bash / WebFetch / MCP tool 等）の結果に、主要な主張が裏付けられている
...
- 質問への回答と、その回答に基づく作業実行を混ぜない。作業は指示を受けてから着手する
- 質問は「答えで次の行動が変わる」ものに絞る。文章で聞くときは1度に1問（複数は件数宣言して順に）。AskUserQuestion は独立した関連決定なら複数問まとめて可、相互依存する質問は逐次
